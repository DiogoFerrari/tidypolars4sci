...
To create new variables based on the transformation of existing ones. Here is the ~starwars~ dataset provided with the module:

#+BEGIN_SRC python :exports both :results output code :tangle src-mutate.py :cache yes :noweb no :session *Python* :linenums 1 :eval always
...
#+RESULTS[cdf96922c670ab82ce844b8ebc8d4193ccc2709b]:
#+begin_src python
...
#+RESULTS[b75d34545d1a40007dbaf382846bf1f3102ad105]:
#+begin_src python
shape: (5, 11)
┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
...
To create new variables and store the results in a new ~tibble~ called ~df~:
# 
#+BEGIN_SRC python :exports both :results output code :tangle src-mutate.py :cache yes :noweb no :session *Python*  :linenums 1
df = (starwars
...
#+RESULTS[b60bd25ef3a0261ade7a06a6bdde2f93a5a7b439]:
#+begin_src python
shape: (5, 13)
┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
...
#+RESULTS[1333f02acd9cb19326dad13849571acb2019f316]:
#+begin_src python
shape: (5, 11)
┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
│ name             height     mass   hair_color   skin_color    eye_color   birth_year   sex      gender      homeworld   species │
...
#+RESULTS[ec55761a8d31ba2033a5903c6bf480f78cc7f63a]:
#+begin_src python
shape: (5, 4)
┌───────────────────────────────────────────────┐
...
The module provides many other default functions to compute summary statistics of columns and use them in combination with ~mutate()~ or ~summary()~. Check [[file:../statistics/summarize.md][Summarize]] in the User Guide and [[file:../../../reference/stats/abs/][Summary Statistics]] in the API reference for more information.


** Custom functions

The function ~map()~ can be used to apply user-defined custom functions. The result can be stored in a new column of the data frame using ~mutate()~. Here is an example of how to apply custom functions to one or more columns:

- Note: the ~*~ in ~*cols~ is used to expand the columns. One could also use ~cols[0], cols[1]~ instead. The index follows the order of the column names provided in the list before the ~lambda~ function.
...
To create variables by group, use ~group_by()~. Here is a summary by four groups ~"vs", "am", "gear", 'carb'~:
...
#+RESULTS[2a93d32520faa5f5071c77d6db7fa08351445c38]:
#+begin_src python
shape: (5, 17)
┌────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
...
#+RESULTS[35c0ced3072cd4199108aecf7a9185598c77d1f6]:
#+begin_src python
shape: (5, 12)
┌────────────────────────────────────────────────────────────────────────────────────────────────┐
...
To change variables in the data that contain either "ar" or "dr" in their name to categorical type and store the results in the variables <original name>_cat, use:
...
#+RESULTS[7f335247bcac137ba780fa4432f3043c070d1e5c]:
#+begin_src python
shape: (5, 15)
┌─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
...
#+RESULTS[5d92336347ad8540c905820d09656620460b2c66]:
#+begin_src python
shape: (5, 12)
┌────────────────────────────────────────────────────────────────────────────────────────────────┐
│ name                  mpg   cyl     disp    hp   drat     wt    qsec    vs    am   gear   carb │
...
#+RESULTS[52b16f96b67e1b04f24beb091ee93a3088c01e06]:
#+begin_src python
shape: (5, 12)
┌────────────────────────────────────────────────────────────────────────────────────────────────┐
│ name                  mpg   cyl     disp   hp    drat     wt    qsec   vs    am    gear   carb │
...
#+RESULTS[92f47486d44e22879c8d0e8b9c0d606447381065]:
#+begin_src python
shape: (5, 14)
┌───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
...
#+RESULTS[3244e59376f4f8fe0190a9c33ffa6c357ccfc9ee]:
#+begin_src python
shape: (5, 12)
┌───────────────────────────────────────────────────────────────────────────────────────────────┐
│ name                mpg    cyl   disp    hp    drat   wt      qsec    vs    am    gear   carb │
